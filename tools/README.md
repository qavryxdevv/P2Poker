...
| `package-linux.sh` | `D-078`: the Linux packages of a built client — an AppImage, a `.deb`, an `.rpm` and a `.tar.gz` — into `dist/`. The AppImage's tool and runtime are fetched at pinned versions and refused unless their SHA-256 is the one in the script. The `.deb` and the `.rpm` carry the AppStream metainfo a software centre lists them by (`S1-JA`); a second argument is the packaging's number, for a released program packaged again (`0.1.4 2` makes `p2poker_0.1.4-2_amd64.deb`), which `.github/workflows/repackage-linux.yml` does. |
| `check-linux-listing.sh` | `S1-JA`: once the `.deb` is installed on a Linux runner, does a software centre list it? AppStream's validator on the tree the packages are made from, dpkg's word on what the package installed, and the system's AppStream asked for the component. Run by `release.yml` and `repackage-linux.yml`. |
