...
ABORT = re.compile(r'every stack is restored')
...
def fold(run):
    ...
    c = dict(aborts=0, own=0, peer=0, not_all=0, disagree=0, refused=0, lines=0)
...
def main():
    ...
    tot = dict(aborts=0, own=0, peer=0, not_all=0, disagree=0, refused=0, runs=0)
    silent = 0
    for run in want:
        ...
        c = fold(run)
        tot['runs'] += 1
        for k in ('aborts', 'own', 'peer', 'not_all', 'disagree', 'refused'):
            tot[k] += c[k]
        ...
        if len(want) <= 6:
            ...
            if c['own'] + c['peer'] == 0 and c['disagree'] + c['refused'] == 0:
    ...
    print('%d run(s): aborts %d; late closes -- own %d (over a dissent %d), peer %d; '
          'disagreements %d; refusals %d'
          % (tot['runs'], tot['aborts'], tot['own'], tot['not_all'], tot['peer'],
             tot['disagree'], tot['refused']))
    if silent:
