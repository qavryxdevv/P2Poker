...
| `DECISIONS.md` **D-028** | **the roster grows only by a return certificate, and this document consumes nothing from it.** `RETURN_VOTE` (`0x0603`) and `RETURN_CERT` (`0x0604`) are protocol-layer artefacts of a hand's **boundary** (`PROTOCOL.md` §4.8, §8.3.1): `R(k+1) = ((R(k) \ OUT(k)) ∪ IN(k)) ∩ ALIVE(k+1)`, both directions certificate-gated. Nothing here moves: `readmit` stays §4.9's `A` and widens only the accepted set of `HAND_INIT(k+1)`; `dealt_in ⊆ P(m)` is exact as before; a returned seat reaches this layer only as a member of the next `HAND_INIT`'s required set, dealt in with its allowance refilled, and the engine cannot tell it from a seat that was never out. **T67** and §5.3 step 4 are unchanged |
| `DECISIONS.md` **D-029** | **a session survives its client's restart, and this document consumes nothing from it.** The session record, the rejoin and the hand adopted from the members' `HAND_INIT` copies (`PROTOCOL.md` §4.4's box, D-029) are the node's and the protocol layer's: an adopted hand enters this engine exactly as any hand does, from its `HAND_INIT`, with the stacks, positions and blinds that body names, and the engine cannot tell it from a derived one. The restarted seat is a bystander until a return certificate (D-028) makes it a member; nothing here reads whether a hand was adopted |
| `DECISIONS.md` **D-030** | **the table screen and the seats' chat consume nothing from this document and feed nothing into it.** `NodeEvent::TableState` is a reading of the engine after each action -- `Hand::bets`, `stacks`, `pot`, `folded`, `turn` -- and `TABLE_CHAT` is an unchained message outside every chain; the engine cannot tell a table with a window from one without, and no state transition reads a chat line, a mute, a ping or the hero's odds |
