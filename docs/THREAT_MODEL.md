...
4. **Superseded by D-015.** The point read that equivocation proofs and timeout
   certificates *"may still be **produced** — they are how a human or a later
   version adjudicates"*, while consuming one never moves a chip or removes a
   player. **D-015 stops producing them.** Points 1–3 stand unchanged and are
   what still binds every classification below; point 4's surviving half is the
   half about consumers, because a later version that reinstates the producer
   must not reinstate a consumer with it.

**D-015 answers `OQ-F` and it changes what several rows below are *about*.**
`TIMEOUT_VOTE`, `TIMEOUT_CERT` and `EquivocationProof` are **defined on the wire
and not produced in version 1**: nothing emits one, and a receiver drops one
unopened (`PROTOCOL.md`'s header box, §4.8, §5.2.4). `STATE_MACHINE.md` deletes
the six transitions that consumed a certificate and the event variant with them.
**The classification consequences are gathered once, in §5.3's D-015 block, and
the three costs are standing limitations in §9.1.0 item 5** — including the one
that is the real loss: an equivocation is now **detected and not provable to a
...
Deadlines never enter the engine as a wall-clock read, and the engine contains no
clocks. **Since D-015 no signed artefact of time enters it either**, which
strengthens the assumption rather than weakening it: the sentence that stood here
said *"time enters only as a signed **timeout certificate** (**D-006**)"*, and no
certificate is produced. Time now reaches the engine as two ordinary chained
events — a seat's **own** auto check/fold, signed by the seat that owed the
action, and the terminal `HAND_ABORT` on `hand_deadline_ms` — neither of which is
anybody's assertion about a clock. **[R2]** The certificate's fields, its emitter
set and its stage shape are `PROTOCOL.md` §4.8's and `STATE_MACHINE.md` §8.4's,
retained there as a specification nothing implements; the description this
...
though it bound anybody. What this assumption asserts is only the negative, and it
is now met by construction: **no peer's unsupported word about the clock changes
any state, because there is no message by which a peer can offer one.**
...
* ~~produce a **timeout certificate** for the honest player~~ — **not in version 1
  (D-015): no certificate is produced, so a coalition has no artefact to build.**
  The capability is recorded because it returns with the machinery, and because
  the reason it was a capability at all is the reason the machinery is hard: the
  required voter set is every *other* dealt-in seat, so a majority coalition
  **is** that whole set (see X10 in §5 — a genuine attack, classified honestly,
  and reclassified **CP-by-absence** in §5.3's D-015 block). Note that
...
| 3 | `TIMEOUT_VOTE` | two required votes about two simultaneous subjects (**X31**) | `subject_seat` in the key (D-009 rule 1); **and, since D-015, no honest peer emits either vote** — the axis is retained in the key regardless, because narrowing it is the mistake this table records five times |
...
**Reclassified by D-015, and for the third time the mechanism is what changed rather than the
defence.** D-010 deleted the payoff, D-011 rule 3 deleted the escalation, and **D-015 deletes
the artefact**: `TIMEOUT_VOTE`, `TIMEOUT_CERT` and `EquivocationProof` are defined on the wire
and produced by nothing (`PROTOCOL.md`'s header box). Every row below whose attack **is** the
construction of one of those objects, or whose mitigation **was** one of them, is re-classified
here. The direction is not uniform and that is the point of the table: three rows get better,
two get worse, and one is unclassifiable in the old scheme and needs the honest label.

| Row | What the attack needed | Under D-015 | Class before | Class after |
|---|---|---|---|---|
| **X10** — forged certificate by all other dealt-in seats, stealing the subject's action | a `TIMEOUT_CERT` the coalition signs | **there is no certificate to sign.** A coalition that is the whole voter set has no message with which to take the subject's action; the subject's client emits its own auto check/fold, or the stage stalls to `hand_deadline_ms` and the hand is voided neutrally | DNA | **CP** — *structurally excluded*, and §5.1's definition is met exactly: no message in the protocol's grammar expresses the attack. **The only row D-015 moves in the good direction, and it is the row OQ8 asked an unanswerable question about** |
| **X30** — shrinking `V` to one seat and taking the subject's chips | votes to shrink `V`, then a single-signature certificate | **neither message exists.** The inductive exclusion rule that closed it is retained in `PROTOCOL.md` §8.3 as specification, and nothing depends on it | D&A | **CP** — structurally excluded. **The label moves, and the reason it is worth recording is that the fix that closes it is not the fix that was designed for it**: five passes hardened `V`, and what finally closed the row was removing the message |
| **X31** — two Sybil seats make an honest voter's own two required votes an `EquivocationProof` against it | the honest peer to emit two `TIMEOUT_VOTE`s | **the honest peer emits none**, and no proof object is built from any pair. The subject axis in `PROTOCOL.md` §5.2.1's slot key is retained regardless, because narrowing that key is how five passes went wrong | D&A | **CP** — structurally excluded. **This does not repair the property it violated**: §9.1.0 item 3 is about the *message set*, and removing two message types from the set does not make the discipline structural for the ones that remain (limitation 12) |
| **X7** — withholding decryption shares to force an abort | nothing; silence | **unchanged as an attack; its classification collapses to one case.** The row's three-way split — D&A for one silent seat at `\|V\| >= 2`, DNA otherwise — read whether the transcript *names* anybody, and naming needed a certificate. Nothing names anybody now | D&A / DNA | **DNA**, uniformly, at every table size. **A row that got worse**, and the loss is D-015's first stated cost: no signed record of who timed out |
| **X8** — the rage-quit escape | nothing; silence | **unchanged in outcome, cheaper in evidence.** The escape was already open everywhere under D-010; what it loses is the sentence *"where a certificate can attribute, it is named outright"* — no abort of this version names anybody on `cause = 1` | V | **V** (unchanged label, **the attribution half of mitigation (i) deleted**) |
| **X16** — sending different histories to different peers (equivocation; §17 row 16) | the equivocation itself | **detected exactly as before, and no longer provable to a third party.** §5.2.1's key and §5.2.2's predicate are untouched: the second body is still rejected, the peers that accepted different copies still diverge, and the hand still ends through the chained path. What is gone is the self-contained artefact that carried the two signed bodies to somebody who was not at the table | D&A | **D&A with its second half struck** — *detected and named to the participants*, never *transferable*. §5.1's D&A explicitly promises evidence "transferable to third parties", and that promise is now false for this row, so the row is marked rather than left to inherit a bucket definition it no longer satisfies |

**The X16 row is the one to read twice, and §9.1.0 item 5 is where it is carried as a
limitation rather than a cell.** Everything else in this table is a payoff or an artefact
disappearing. X16 is a **property of the design being withdrawn**: this corpus has said, since
the first pass, that the strongest evidence the protocol can produce is a self-contained
equivocation proof checkable by anyone with two byte strings and Ed25519, forever, needing no
table state. That artefact is not produced. Detection is unchanged and consequence was already
nil under D-010, so nothing that ever *happened* changes — but a claim this document made is no
longer true, and `SPEC_CS.md` §18 requires it to be withdrawn in the same words it was made in
rather than quietly narrowed.

**Two rows are deliberately *not* in this table.** **X29** (fault any table with a false
`state_hash`) never involved a certificate or a proof — its evidence is the unanimous transcript
plus every peer's signed `STATE_HASH`, all of it still produced. **X32** (the terminal
`HAND_ABORT` framing its own emitter) was closed by putting `event_type` in the slot key, and
the key is retained whole; the row's honest peer emits an abort, not a vote, so D-015 does not
reach it. A reader who expects every X-row that mentions the word *certificate* to appear above
should check which side of the mechanism the row sits on: this table is for rows whose **attack
or mitigation was the object**, not rows that merely cite a rule about it.

**And the counts, because §5.4 reads them.** Three rows move into **CP**, one moves out of
**D&A** into **DNA**, and one keeps its label with a documented hole in it. **That is a net
improvement in the table and it must not be read as a strengthening of the design**: nothing
was defended better; three attacks lost the message they were made of, and the design lost a
capability in exchange. A review that works makes the proportion classified as prevented fall,
and this pass makes it rise for the one reason that is not a warning sign — the mechanism the
attacks targeted is gone, along with what it was for.
...
| X4 | Encoding equivocation: two byte encodings of one logical event, signed separately, sent to different peers | **CP** | **[R21]** The canonical encoding and the gate that enforces it are `PROTOCOL.md` §2.8's, and the gate's measured behaviour is `research/CRYPTO_LIBS.md` §4.7's — *The canonicality gate*, which is the section this row named `CRYPTOGRAPHY.md`'s by slip; that document has no §4.7 and §4 is *The group, and why* — the container shape, the field-order rule, the position of the gate relative to the signature check and the append-only field-index rule are all theirs, and the description this cell carried is deleted under D-011 rule 1. What is classified here is that the attack is **structurally excluded**: canonicalisation is decided by the encoding rules and checked before any signature is verified, so a second byte encoding of one logical event is rejected at the gate rather than reaching a slot. The measured evidence for the gate is this document's to keep, because it is evidence rather than definition — all three hostile encodings were caught: non-preferred integers (`8218011a00000018`), indefinite-length arrays (`9f…ff`), and trailing bytes; truncated inputs are all rejected without panic (`research/CRYPTO_LIBS.md` §4.7). The one thing this row does add, because it is a *threat* observation rather than an encoding rule: reusing a field index would silently change the meaning of historical signed bytes, so the append-only discipline `PROTOCOL.md` §2.8 states is load-bearing for G6 and not a style preference. *Inherits A5.* |
...
| X7 | Withholding decryption shares to force a hand to abort (griefing) | **DNA**, uniformly, since D-015 (was **D&A / DNA**) | **The three-way split below is superseded and is retained as the reasoning a later version restores.** Under D-015 no certificate is produced, so no silent seat is ever named on `cause = 1`, at any table size and whatever `\|V\|` would have been — the row is DNA in every case. The attack, its cost and its chip outcome are unchanged; what is gone is attribution.<br><br>Permanent and inherent to the n-of-n construction: any player who goes silent makes it impossible for anyone to open any further card. The missing `REVEAL_TOKEN` for a given `(hand_id, card_index)` is publicly visible and every other player's signed events prove they did their part. **Attribution depends on the case and is not uniform:**<br><br>*one silent seat, with `\|V(subject)\| >= 2`* — **D&A**;<br>*two or more seats silent simultaneously* — **DNA**;<br>*any silent seat where `\|V(subject)\| < 2`, which at `n = 2` is always* — **DNA**.<br><br>Attribution is sound for a single silent seat whenever the required voter set still has two or more members; it is unavailable when two or more seats stop at once, because each required voter set contains the other subject and D-008 forbids removing a seat from `V` on anything short of a completed certificate (`PROTOCOL.md` Q-02, `STATE_MACHINE.md` Q3, OQ-E) — **resolved by D-036 (2026-09-11): several quiet seats are named together by one certificate whose voters are every other live seat, provided the voters outnumber the named; below that bar the case stays DNA and the hand ends on its deadline naming nobody**, and unavailable whenever `\|V\| < 2` for the reason in D-007 as generalised by D-008. **It cannot be prevented, and the fix that would prevent it is forbidden** — see §7. **Under D-010 the three cases have identical chip outcomes**: every abort restores every stack to its start-of-hand value, so the silent seat keeps its commitment in all three, and the split above now records only whether the transcript *names* anybody. That distinction no longer changes any outcome, because attribution carries no automatic consequence — it is kept in the split because naming is still what a human or a later version would adjudicate on, and because a document that collapsed the cases would be claiming an attribution property it does not have. The attacker gains no cards. What it gains is a wasted hand; where the silent seat is also the losing seat, that is X8's escape rather than pure griefing, and X8 is where the cost is classified. Repeated aborts attributable to one identity are visible to everyone and counted per identity in the lobby; in play money that is the whole penalty, and `SPEC_CS.md` §18 forbids claiming more. |
...
|---|---:|---:|---:|
| Cryptographically prevented (CP) | 11 | **9** | **20** |
| Detected and attributed (D&A) | 7 | **6** | **13** |
| Detected but not attributable (DNA) | 0 | **8** | **8** |
| ~~Split D&A / DNA by case (X7)~~ — **retired by D-015; X7 is DNA in every case** | 0 | 0 | **0** |
...
**D-015 moved four rows and retired a split row, and it is the only pass that moved rows *into*
CP.** X10, X30 and X31 leave D&A and DNA for **CP by structural exclusion** — the attacks are
constructions of a `TIMEOUT_CERT` or an `EquivocationProof`, and no message in the grammar
expresses them now that nothing produces one (§5.3's D-015 block). X7's D&A/DNA split is retired
and the row is **DNA in every case**, because naming a silent seat needed the certificate. The
extended catalogue stays at **36** and the combined total at **55** — no row is added or
removed, only re-bucketed. **This is the one movement in this table that must not be read as the
design improving**, and the discipline the whole section is written on says why: the CP column
rose because the *message set shrank*, not because anything is defended better, and the design
gave up a capability to get it (§9.1.0 item 5). **X16 keeps its D&A label with a hole in it** —
§5.1 defines D&A as evidence *transferable to third parties*, and equivocation evidence is no
longer transferable; the row is marked rather than moved, because it is still detected and still
named to the participants, and there is no bucket for *detected, named locally, not
transferable*. Inventing one for a single row would be the error §5.1 warns about in the
opposite direction, so the hole is carried in the cell and in §9.1.0 item 5(a) instead.
...
**And read all three under D-015, which is where case (a) stops needing a certificate at all.**
Nothing produces a `TIMEOUT_VOTE` or a `TIMEOUT_CERT`. Case (a)'s auto check/fold survives — it
is emitted by the **absent player's own client**, which is cooperating throughout, and it is an
ordinary signed action by the seat that owed one. Case (b) is untouched. Case (c) keeps its one
chip outcome and loses its ability to name anybody: the three-way split there survived D-010
*only* as a statement about attribution, and attribution needed the certificate, so the split is
retired and every silent-seat abort is unattributed (§9.1.0 item 5(b), X7).
...
out. **No timeout of any kind ends the tournament or the cash game** (D-006).

**Who emits the auto-action is the sentence D-015 changes, and it is the sentence the whole
case turns on.** The paragraph above says the auto-action is *"a real signed protocol event
that every peer derives identically"*, and under D-006 to D-008 that meant a certificate the
**other** seats assembled, whose deadline had to be agreed without a trusted clock, whose
forgery by all the other dealt-in seats was X10, and whose voter set could be shrunk by
assertion until D-008 (X30). **None of that machinery is built** (D-015). The auto-action is
emitted by the away player's **own client**, which by this case's own hypothesis is running and
cooperating: it signs `ACTION_CHECK` or `ACTION_FOLD` at its own stage when its own timer
expires. That is single-writer by the seat itself — no vote, no voter set, no unanimity, no
shared clock, and **no way for any other seat to produce one**, which is X10 and X30 removed
rather than defended (§5.3's D-015 block).

**Two things are lost with the certificate and both are stated rather than absorbed.** There is
no `was_auto` marker, because an action a seat signs itself is indistinguishable on the wire
from one its human took and no peer may guess; `consecutive_auto_actions` therefore never
increments and **a seat is never marked sitting out by the deadline path at any table size**, so
case (a) no longer flows into case (b) on its own — an away player's stack simply drains on the
blinds, which is the same end state one orbit at a time. The paragraph that stood here scoped
that loss on `|V(subject)| < 2` and called heads-up *"the first mode this project ships … the
one in which this machinery does not apply"*; **it now applies to no mode**, which is a widening
of a stated limitation and not of an exploit. See X10, X30, §9.1.0 item 5, §9.1.2 limitation 3,
and `STATE_MACHINE.md` §8.5 and §9.5.
...
its own record with its **listen** addresses as well; until 2026-09-18 those were
LAN and virtual-adapter addresses too, and since then the public DHT's behaviour
is told of no listen address the bogon filter refuses, so it holds public ones and
circuits only (`NETWORK_STACK.md` §3.5, `S1-FJ`). The same union went out in
...
4. **Linkage across time, without dialling anybody.** Under Mainline an observer
   ...
   bound to a physical location**; until 2026-09-18 with the LAN layout attached
   for anyone this client answered (`S1-FJ`, closed that day).
...
* **Hide the listen addresses** from both doors. The `identify` banner has been
  ...
  `libp2p-kad` hands out when it answers a lookup, which that setting does not
  reach, since 2026-09-18 — the wrapper around the public DHT's behaviour tells it
  of no home address it listens on (`S1-FJ`).
...
#### 9.1.0 The five standing limitations, stated once

Five properties are load-bearing, are each the subject of a numbered owner
...
elaboration appears to, this block wins. **Item 5 is new with D-015** and it is
the only one of the five that records something the design *had* and gave up.
...
5. **The timeout machinery is not produced, and the three costs of that are
   stated here rather than left in the decision (D-015).** `TIMEOUT_VOTE`,
   `TIMEOUT_CERT` and `EquivocationProof` keep their wire definitions and their
   code points, and **nothing emits or consumes one** (`PROTOCOL.md`'s header
   box; `STATE_MACHINE.md` deletes the six transitions and the event variant).
   The measurement that decided it: 26 of the 28.3 MiB per-hand worst case of
   the anti-replay store were the two timeout classes — the client's largest
   attacker-influenced allocation, held for machinery D-010 had already made
   consequence-free. What it costs, in the order of how much it costs:

   *(a) An equivocation is detected and **not provable to a third party who was
   not present.** This is the real loss and it is a **withdrawn claim**, not a
   reduced payoff.* The predicate is untouched, so two bodies in one slot are
   still found, the second is still rejected, the peers that accepted different
   first copies still diverge, and the hand still ends through the chained path
   — detection is exactly what it was. What is gone is the **self-contained
   artefact**: the object a checker could verify with two byte strings and
   Ed25519, no table state, no transcript, no knowledge of the game, forever.
   Every claim in this corpus that equivocation evidence is *transferable to
   third parties* — including §5.1's definition of the **D&A** bucket, in those
   words — is false for equivocation in this version and must be read with this
   item beside it. `SPEC_CS.md` §18 forbids narrowing a claim quietly, so it is
   withdrawn in the words it was made in. **It is the one cost of D-015 that is
   not settled**: `DECISIONS.md` **D-015-2** requires a numbered decision before
   real money on whether to accept it or reinstate the proof object with the
   machinery reviewed. Catalogued at X16 in §5.3's D-015 block.

   *(b) There is no signed record of who timed out.* No abort of this version
   populates `attributed` on `cause = 1`, at any table size: the certified-subject
   path is the only one that ever did and it is unreachable. What a transcript
   still shows is the stage that stalled and whose event is missing from it —
   unsigned, legible to anyone holding the transcript, and exactly what a human
   or a later version would have read anyway, since under D-010 nothing acted on
   the signed form either. **X7 is the row that moves for this**, from a
   three-way D&A/DNA split to DNA at every table size.

   *(c) A later version cannot adjudicate today's transcripts* for timeout
   questions, because the evidence was never produced. Accepted without
   mitigation: D-010 had already made the evidence consequence-free, so nothing
   was going to be adjudicated from it, and producing an artefact **on the
   chance** that a later version wants it is what the 26 MiB were being spent on.

   **What D-015 does *not* cost, checked because the obvious reading is wrong.**
   It strands nobody at the keyboard. A player who has walked away is still
   running a cooperating client: it publishes decryption shares automatically and
   it emits its **own** auto check/fold when its own timer expires — single-writer
   by the seat itself, no vote, no quorum, no shared clock. A client that emits
   nothing at all stalls one hand to `hand_deadline_ms` and is then outside the
   next hand's required set under D-013. Those two cases are §7.3's two columns,
   they are covered by two mechanisms that already existed, and the certificate
   sat between them covering neither. **And it removes three attacks outright** —
   X10, X30 and X31 become CP by structural exclusion (§5.3), which is not a
   defence added but a message set reduced.
...
3. **Action deadlines are not enforceable against a determined opponent, and
   since D-015 the failure mode no longer differs by anything** (X10). This
   elaborates §9.1.0 items 1, 2 and 5. **Read the D-015 paragraph at the end of
   this item first**: the two cases below are the cases of a mechanism that is
   not built, kept because a later version restores them and because the reason
   the `|V| < 2` case was hopeless is the reason the whole mechanism was
   deferred. Where
   ...
   **Under D-015 both cases collapse into one and the honest statement is
   shorter.** No certificate is produced, so the `|V| >= 2` case — a coalition
   of every other dealt-in seat stealing an honest player's action — **has no
   message to be built from**, and X10 becomes CP by structural exclusion. What
   is left is the `|V| < 2` behaviour, applied everywhere: **a stalling opponent
   cannot be punished inside the protocol at any table size, the hand ends at
   `hand_deadline_ms` with nobody named, and the only remedy is to leave the
   table.** That is a *widening of a stated limitation*, and it is worth being
   precise about what it buys and costs: it costs nothing an attacker can use
   — the certificate's whole effect was one hand's action, D-010 having already
   removed its chips — and it buys the removal of the one construction by which
   a coalition could act *against* an honest player through the deadline path.
   A player who has merely walked away is unaffected either way, because their
   own client folds them (§9.1.0 item 5).
4. **There is no automated forfeiture and no automated eviction, and the price of
   ...
   *What is removed.* An abort is neutral: stacks are restored to their
   ...
   D-011 rule 3 deletes all three. **The sentence that followed — *"equivocation
   proofs and timeout certificates are still produced — they are how a human or a
   later version adjudicates"* — is withdrawn by D-015**: they are not produced,
   and §9.1.0 item 5 carries what that costs. What survives is the clause about
   consumers, and it is the one a later version must not undo: consuming one
   never moves a chip, removes a player, or costs anyone a connection.
...
12. **That honest behaviour never manufactures evidence against the honest peer
    ...
    **D-015 removes two of the five message types from the set and does not
    repair the property either — it makes the remaining discipline smaller, not
    structural.** `TIMEOUT_VOTE` is no longer produced, so recurrence #3 cannot
    recur; and no `EquivocationProof` is built from any pair, so **a sixth
    recurrence would no longer frame anybody to a third party** — which sounds
    like a repair and is not. The failure this limitation names is a
    *specification* failure: a slot key coarser than the behaviour the protocol
    requires means two honest emissions collide, the second is **rejected**, and
    the honest peer's own conduct stops its own hand. That happens with or
    without a proof object, because §5.2.1's key and §4.0 step 10a are what
    reject it. So the property is exactly as unenforced as it was, over a
    message set of 37 emitted types instead of 39, and the mirror test of §5.5
    is still the only thing checking it.
...
| R21 | X4 | the canonical container shape, the field-order rule, the gate's position relative to the signature check, the append-only index rule | `PROTOCOL.md` §2.8, `research/CRYPTO_LIBS.md` §4.7 |
...
| ~~**OQ-F**~~ | **CLOSED by D-015: they are not produced.** The question was *"Whether `TIMEOUT_VOTE`, `TIMEOUT_CERT` and `EquivocationProof` should still be produced in the MVP now that D-010 gives them no effect, or be deferred wholesale until the machinery is sound. Producing them keeps the transcript adjudicable later; deferring them removes four passes' worth of surface."* | **The last clause of this cell is withdrawn.** It read that the letter *"changes what ships rather than what is true, so no classification here turns on it"* — and six classifications turned on it: X10, X30 and X31 move to **CP**, X7 moves to **DNA**, X8 loses half a mitigation, and X16's D&A loses its transferability. §5.3's D-015 block carries all six. **The error is worth naming because it is a recurring one in this file**: a decision about *what ships* is a decision about which messages exist, and a classification scheme whose first bucket is *"no message in the grammar expresses the attack"* cannot be indifferent to that. The successor question is **D-015-2** — whether an equivocation being unprovable to a third party is acceptable before real money — and it is carried in §9.1.0 item 5(a), where the cost is, rather than as a letter. |
