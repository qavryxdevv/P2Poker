...
| S1-F | **The client cannot play a ten-seat table, and `MAX_SEATS` is ten. Measured, not inferred.** `tools/table-run.ps1` starts N headless `--autoplay` clients and reads back what **every** node saw. Tox path, one machine, 300 s a run: **3 seats 7.7 s/hand, 6 seats 10.3, 7 seats 11.2, 8 seats 12.9** — a gentle slope, every seat in the group and finishing hands. Then **9 seats: 42.3 s/hand**, the founder opening 6 hands while six joiners opened 10 (*the founder behind its own table*) and one seat opening four hands on a genesis nobody else held; **10 seats: 61.1 s/hand**, two seats never in the Tox group and three finishing no hand at all. Nothing refuses such a table — `MAX_SEATS` is 10 and `hand_deadline_min_ms` is derived for 10 — so the protocol permits a size the client does not play. **Two things the measurement had to learn, and both are about evidence.** First, **`TABLE FORMED session=… seats=N` is not evidence that a seat played**: it reports the roster a peer holds, and a peer that heard no hand still holds it. A seat invited into the group at **100 s** (it asked to join at 1.5 s) opened its own hands, played none, and printed `TABLE FORMED` with the same session id as the seats that were playing — and that line is the pass condition every scripted test in this repository reads back. Second, **matching genesis hashes are not evidence either**, which the first version of the harness got wrong: `genesis_hand` is a hash of the table id, hand number, session, roster hash, terminal-zero and ratifiers, all held the moment the roster ratifies, so two peers agree on hand N's genesis without exchanging a message. At ten seats a node that never entered the group agreed on all five hands it opened and finished none. **Opening a hand is local; finishing one is not.** **One cause found and fixed.** The founder invited a seat only from the `FriendConnection` up-edge, and `invited` is appended to only when `tox_group_invite_friend` succeeds — so a refused invitation was never retried, the trigger having passed, and the next up-edge arrives only when the connection drops and returns. It is now a swept condition (connected AND on the roster AND not yet invited, every five seconds) as well as an edge, with `pending_invites` split out and pinned by a test whose third case is the one that was wrong, and a `Trouble::invites_refused` counter because **a seat arriving late is two failures that look identical in a log** — a friend connection not yet up, which only waiting fixes, versus an invitation refused. **The fix is not the whole cliff.** The founder falling behind its own table at nine seats is not an invitation problem, and the transport counters were **zero** through the ten-seat run — no send refused, nothing queued — so it is not the transport. | `src/net/run.rs`, `src/table/hand.rs` — **open; eight seats plays and nine does not, and where the founder's time goes at nine is the next measurement** |
