...
**What this does not do**, so the amendment is not read wider than it is:
`say_again` repeats only a client's **own** ratification, and `Formation::ratified`
stores hashes rather than bytes, so no peer holds a third party's copy and none
can repair one on its behalf. That residue is `S1-P`.
