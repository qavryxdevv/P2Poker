...
| K-9-a | **New, owed back to `PROTOCOL.md` by K-9's fix, and it is small.** §3.2's second entry condition and §6.3 step 1 both justify the solitary trigger with *"There is no checkpoint on that path to compare hashes at: a hand with one participant reaches no `DECK_COMMIT` and no betting round, so §6.2 places none (that is `K-3` in `DECISIONS.md`'s open list…)"*. **K-3 has landed**: every hand places checkpoint 8 at its boundary, drain hands included. The justification is stale by one pass, and it is not merely editorial — the reconciliation round of §4.9 chains from a checkpoint's `stage_hash`, so the checkpoint 8 that now exists is what gives `STATE_MACHINE.md` T53 something to release the solitary freeze with. Restate the trigger's justification as *the checkpoint exists but the divergence is not a hash mismatch* rather than *no checkpoint exists*. | `PROTOCOL.md` §3.2, §6.3 — low |
