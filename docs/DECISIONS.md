...
| G6-R6 | `HEADS_UP_PLAY_MONEY_V1` is a named preset that no document defines. §9.4 rule 3 makes a preset *name* imply exact values and only `RATED_SNG_POKERTH_V1` is pinned, so two clients could ship different heads-up tables under one name and fail to join each other - the same identity hazard as G6-R1, which has already happened once. It exists in `src/poker/tournament.rs` because §32 requires heads-up first and the rated preset cannot be played two-handed. Either pin it in §13 or drop the name and treat it as an ordinary custom advert. | `PROTOCOL.md` |
